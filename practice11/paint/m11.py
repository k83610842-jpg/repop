...
# Квадрат: берем меньшую из сторон и рисуем квадрат по ней
...
# Прямоугольный треугольник: прямой угол в точке нажатия
def get_right_triangle_points(x1, y1, x2, y2):
    return [(x1, y1), (x2, y1), (x1, y2)]

# Равносторонний треугольник: основание горизонтальное, высота считается через sqrt(3)/2
...
# Ромб: центр в точке нажатия, вершины по четырём сторонам
...
# Вспомогательная функция: рисуем любой многоугольник по списку точек
...
def main():
    ...
    # Все режимы у которых есть «призрак» при перетаскивании
    shape_modes = ['rect', 'circle', 'square', 'right_triangle', 'eq_triangle', 'rhombus']
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                # 5-квадрат, 6-прямоугольный треугольник, 7-равносторонний треугольник, 8-ромб
            ...
            if event.type == pygame.MOUSEMOTION:
                if LMBpressed:
                    ...
                    if mode == 'brush':
                        ...
                        # Новые фигуры — призрак при перетаскивании
                        elif mode == 'square':
                            draw_polygon(screen, color, get_square_points(startX, startY, *curr_pos))
                        elif mode == 'right_triangle':
                            draw_polygon(screen, color, get_right_triangle_points(startX, startY, *curr_pos))
                        elif mode == 'eq_triangle':
                            draw_polygon(screen, color, get_equilateral_triangle_points(startX, startY, *curr_pos))
                        elif mode == 'rhombus':
                            draw_polygon(screen, color, get_rhombus_points(startX, startY, *curr_pos))
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                if event.button == 1:
                    ...
                    # Фиксируем новые фигуры на холсте при отпускании кнопки
                    elif mode == 'square':
                        draw_polygon(base_layer, color, get_square_points(startX, startY, *event.pos))
                    elif mode == 'right_triangle':
                        draw_polygon(base_layer, color, get_right_triangle_points(startX, startY, *event.pos))
                    elif mode == 'eq_triangle':
                        draw_polygon(base_layer, color, get_equilateral_triangle_points(startX, startY, *event.pos))
                    elif mode == 'rhombus':
                        draw_polygon(base_layer, color, get_rhombus_points(startX, startY, *event.pos))
                    prev_pos = None
